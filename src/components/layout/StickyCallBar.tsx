...
export function StickyCallBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between border-t border-brand/20 bg-navy px-3 py-2 md:hidden">
      {/* Call button */}
      <a
        href={`tel:${stripPhoneDigits(site.phone)}`}
        className="flex min-h-[48px] flex-1 items-center justify-center gap-1.5 rounded-md bg-signal px-3 py-2 text-xs font-semibold text-ink transition-colors hover:bg-signal-dark"
      >
      ...
      <Link
        href="/contact"
        className="ml-3 flex min-h-[48px] items-center justify-center rounded-md border border-brand px-3 py-2 text-xs font-semibold text-signal transition-colors hover:bg-brand/10"
      >
        Get a Quote
      </Link>
    </div>
